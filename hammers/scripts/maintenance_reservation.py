...
def main():
    ...
    blazardb = mysql.connector.connect(user=blazar_db_user, database='blazar')
    cursor = blazardb.cursor(buffered=False)
    query = ("SELECT leases.end_date FROM leases "
        "INNER JOIN reservations ON reservations.lease_id = leases.id "
        "INNER JOIN computehost_allocations ON reservations.id  = computehost_allocations.reservation_id "
        "INNER JOIN computehosts ON computehosts.id = computehost_allocations.compute_host_id "
    ...
    for node in args.nodes:
        ...
        lease_name = "maintenance_"+node_name
        resource_properties = '["=", "$uid", "'+node_uuid+'"]'
        phys_res = {'min': "1", 'max': "1", 'hypervisor_properties': "", 'resource_properties': resource_properties, 'resource_type': 'physical:host'}
        next_start_str = next_start_time.strftime(time_format)
        next_end_str   = next_end_time.strftime(time_format)

        print "Creating maintenance reservation for node %(node)s, starting %(start)s and ending %(end)s" % {'node': node_name, 'start': next_start_time.strftime(time_format+" %Z"), 'end': next_end_time.strftime(time_format+" %Z")}
        if not args.dry_run:
            blazar.lease.create(name=lease_name, start=next_start_str, end=next_end_str, reservations=[phys_res], events=[])
